...
sub parse {
	...
	# let's see if we're BOTLINK data or not
#	} elsif ($line =~ /^([0-9a-f]{32})\s(\w\w)\s%%(.+)%%$/) {
#	   $ev = Net::IRC::Event->new( "botlink",
#				       undef, 	# no from, we'll determine that from the seqno
#				       undef,	# no to, we're the one it came to
#				       undef,	# no format, though it could be $2 (the request type)
#				       $1, $2, $3 );
